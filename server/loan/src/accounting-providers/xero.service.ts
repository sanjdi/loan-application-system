...
@Injectable()
export class XeroService implements AccountingService {
  getBalanceSheet(companyId: string): BalanceSheet {
    console.log(`Returning balance sheet for company id ${companyId} from Xero`);
    ...
    for (let i = 2023; i > 2022; i--) {
      for (let j = 12; j >= 1; j--) {
        balances.push({
          year: i,
          month: j,
          profitOrLoss: Math.floor(Math.random() * 600001) - 10000,
